...
export function buildPrompt(context, options = {}) {
  ...
  return [
    ...
    '- Pass dirForAssetWrites only when the target tool schema explicitly supports that argument.',
    '- If a tool returns unsupported/unknown argument error for dirForAssetWrites, remove that argument and retry once with the same node-id.',
    '- For tools that do not support dirForAssetWrites, call with only schema-supported arguments (for example nodeId).',
